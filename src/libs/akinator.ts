...
const setAutoCleanup = (id: string) => {
  ...
  clearTimeout(session.timeout);
  const timeout = setTimeout(() => cleanupSession(id, 'inactivity'), 5 * 60 * 1000);
...
export const startSessionAkinator = async (
  ...
  try {
    const connectResult = await connect({
      port: Number(process.env.REAL_BROWSER_PORT) || 43043,
      headless: false,
    });
...
export const answerAkinator = async (
  ...
  try {
    ...
    const result = await new Promise<object | null>((resolve, reject) => {
      ...
      page
        .waitForFunction(() => typeof (window as any).chooseAnswer === 'function')
...
export const cancelAnswerAkinator = async (
  ...
  try {
    ...
    const result = await new Promise<object | null>((resolve, reject) => {
      ...
      page
        .waitForFunction(() => typeof (window as any).cancelAnswer === 'function')
        .then(() => page.evaluate(() => {
